...
echo "<td width='20' align='center'><img src='".$dmshop_coupon_path."/img/arrow.gif' class='up1'></td>";
echo "<td><a href='".$shop['path']."/coupon.php' class='off'>쿠폰 보관함</a></td>";
?>
</tr>
</table>
    </td>
...
<tr>
    <td width="9"></td>
    <td width="104" valign="top"><img src="<?=$dmshop_coupon_path?>/img/t1.gif"></td>
    <td width="10"></td>
    <td width="100"><p class="b1 t2"><span class="t1"><?=number_format($total_count);?></span> 건</p></td>
    <td align="right"><p class="b2 t2">현재 소지하신 쿠폰을 확인 합니다.</p></td>
</tr>
</table>
...
<tr class="bg">
    ...
    <td align="center" class="t1"><b>할인/혜택</b></td>
    <td><img src="<?=$dmshop_coupon_path?>/img/line.gif"></td>
    <td align="center" class="t1"><b>쿠폰명 / 사용조건</b></td>
    <td><img src="<?=$dmshop_coupon_path?>/img/line.gif"></td>
    <td align="center" class="t1"><b>사용기간</b></td>
</tr>
...
?>
    ...
    <td></td>
    <td align="center"><p class="date"><?=date("Y/m/d", strtotime($list[$i]['coupon_date1']));?> ~ <?=date("Y/m/d", strtotime($list[$i]['coupon_date2']));?></p><p class="end">자동소멸 - <?=number_format($list[$i]['coupon_dday']);?>일</p></td>
</tr>
<? } ?>
<? if (!$i) { ?>
<tr><td colspan="7" height="225" align="center"><img src="<?=$dmshop_coupon_path?>/img/coupon_no.gif"></td></tr>
<? } ?>
...
<tr>
    <td class="t1">
- 쿠폰 보관함을 통해 사용 가능한 쿠폰을 확인하실 수 있습니다.<br>
- 쿠폰은 장바구니 또는 주문/결제 페이지에서 각 상품별 1회씩 적용하실 수 있습니다.<br>
- 발행된 사용조건 및 사용기간이 명시되어있으며, 사용기간 만료시 자동소멸 됩니다.<br>
    </td>
</tr>
</table>
</div>
