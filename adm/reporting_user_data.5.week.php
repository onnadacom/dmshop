...
for ($i=0; $i<count($list); $i++) {
    ...
    // 해당 기간의 추천인총수
    $order = sql_fetch(" select count(*) as report_count from $shop[user_table] where dayofweek(datetime) = '".$list[$i]['dayofweek']."' and substring(datetime,1,10) >= '".$date1."' and substring(datetime,1,10) <= '".$date2."' and user_recommend != '' ");

    $list_etc = 0;
    for ($k=0; $k<count($report); $k++) {

        // 해당 기간의 추천인총수을 구한다
        $data = sql_fetch(" select count(*) as report_count from $shop[user_table] where dayofweek(datetime) = '".$list[$i]['dayofweek']."' and substring(datetime,1,10) >= '".$date1."' and substring(datetime,1,10) <= '".$date2."' and user_recommend = '".addslashes($report[$k]['user_recommend'])."' ");
