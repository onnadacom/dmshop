...
for ($i=0; $i<(int)($month_count); $i++) {
    ...
    $list[$i]['month'] = $month;
    $list[$i]['href'] = "./reporting_visit_list.php?date1=".$list[$i]['month']."-01&date2=".$list[$i]['month']."-".date("t", strtotime($list[$i]['month']));
...
for ($i=0; $i<count($list); $i++) {
    ...
    // 페이지 뷰
    $visit_page = sql_fetch(" select count(*) as report_count from $shop[visit_table] where substring(vi_datetime,1,7) = '".$list[$i]['month']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' ");

    // 해당 기간의 가입자 수
    $user = sql_fetch(" select count(*) as report_count from $shop[user_table] where substring(datetime,1,7) = '".$list[$i]['month']."' and substring(datetime,1,10) >= '".$date1."' and substring(datetime,1,10) <= '".$date2."' and user_level >= '2' ");
...
for ($i=0; $i<count($list); $i++) {
    ...
    echo $comma."['".$list[$i]['month']."',".(int)($list[$i]['total_visit_count']).",".(int)($list[$i]['total_visit_page_count']).",".(int)($list[$i]['total_user_regist_count']).",".$list[$i]['total_visit_regist_count']."]";
