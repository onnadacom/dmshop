...
for ($i=0; $i<count($list); $i++) {
    ...
    // 방문자 수
    $visit = sql_fetch(" select count(vi_ip) as report_count from (select distinct substring(vi_datetime,1,10), vi_ip from $shop[visit_table] where substring(vi_datetime,1,4) = '".$list[$i]['year']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."') as x ");

    // 1회 방문
    $visit_return1 = sql_fetch(" select count(vi_ip) as report_count from (select distinct substring(vi_datetime,1,10), vi_ip from $shop[visit_table] where substring(vi_datetime,1,4) = '".$list[$i]['year']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_return = '1') as x");

    // 2회 방문
    $visit_return2 = sql_fetch(" select count(vi_ip) as report_count from (select distinct substring(vi_datetime,1,10), vi_ip from $shop[visit_table] where substring(vi_datetime,1,4) = '".$list[$i]['year']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_return = '2') as x");

    // 3회 방문
    $visit_return3 = sql_fetch(" select count(vi_ip) as report_count from (select distinct substring(vi_datetime,1,10), vi_ip from $shop[visit_table] where substring(vi_datetime,1,4) = '".$list[$i]['year']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_return = '3') as x");

    // 4회 방문
    $visit_return4 = sql_fetch(" select count(vi_ip) as report_count from (select distinct substring(vi_datetime,1,10), vi_ip from $shop[visit_table] where substring(vi_datetime,1,4) = '".$list[$i]['year']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_return = '4') as x");

    // 5회 이상 방문
    $visit_return5 = sql_fetch(" select count(vi_ip) as report_count from (select distinct substring(vi_datetime,1,10), vi_ip from $shop[visit_table] where substring(vi_datetime,1,4) = '".$list[$i]['year']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_return >= '5') as x");
...
function reportingChart()
{
   ...
   var data = google.visualization.arrayToDataTable([
    ['년도', '방문자 수', '1회 방문', '2회 방문', '3회 방문', '4회 방문', '5회 방문'],
<?
$comma = "";
for ($i=0; $i<count($list); $i++) {
    ...
    echo $comma."['".$list[$i]['year']."',".(int)($list[$i]['total_visit_count']).",".(int)($list[$i]['total_visit_return1_count']).",".(int)($list[$i]['total_visit_return2_count']).",".(int)($list[$i]['total_visit_return3_count']).",".(int)($list[$i]['total_visit_return4_count']).",".(int)($list[$i]['total_visit_return5_count'])."]";
