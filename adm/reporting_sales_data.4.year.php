...
for ($i=0; $i<count($list); $i++) {
    ...
    // 네이버 방문
    $visit_host1 = sql_fetch(" select count(vi_ip) as total_count from $shop[visit_table] where substring(vi_datetime,1,4) = '".$list[$i]['year']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_first = '1' and (INSTR(vi_host,'naver.')) ");

    // 다음 방문
    $visit_host2 = sql_fetch(" select count(vi_ip) as total_count from $shop[visit_table] where substring(vi_datetime,1,4) = '".$list[$i]['year']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_first = '1' and (INSTR(vi_host,'daum.')) ");

    // 네이트 방문
    $visit_host3 = sql_fetch(" select count(vi_ip) as total_count from $shop[visit_table] where substring(vi_datetime,1,4) = '".$list[$i]['year']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_first = '1' and (INSTR(vi_host,'nate.')) ");

    // 야후 방문
    $visit_host4 = sql_fetch(" select count(vi_ip) as total_count from $shop[visit_table] where substring(vi_datetime,1,4) = '".$list[$i]['year']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_first = '1' and (INSTR(vi_host,'yahoo.')) ");

    // 구글 방문
    $visit_host5 = sql_fetch(" select count(vi_ip) as total_count from $shop[visit_table] where substring(vi_datetime,1,4) = '".$list[$i]['year']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_first = '1' and (INSTR(vi_host,'google.')) ");

    // 직접 방문
    $visit_host6 = sql_fetch(" select count(vi_ip) as total_count from $shop[visit_table] where substring(vi_datetime,1,4) = '".$list[$i]['year']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_first = '1' and vi_host in ('{$dmshop['doamin']}', '') ");
...
function reportingChart()
{
   ...
   var data = google.visualization.arrayToDataTable([
    ['년도', '방문자 수', '네이버', '다음', '네이트', '야후', '구글', '직접방문', '기타'],
<?
$comma = "";
for ($i=0; $i<count($list); $i++) {
    ...
    echo $comma."['".$list[$i]['year']."',".(int)($list[$i]['total_visit_count']).",".(int)($list[$i]['total_visit_host1_count']).",".(int)($list[$i]['total_visit_host2_count']).",".(int)($list[$i]['total_visit_host3_count']).",".(int)($list[$i]['total_visit_host4_count']).",".(int)($list[$i]['total_visit_host5_count']).",".(int)($list[$i]['total_visit_host6_count']).",".(int)($list[$i]['total_visit_host7_count'])."]";
