...
$contents_box = 500 + (int)(count($icons) * 90);
?>
<style type="text/css">
.contents_box {min-width:<?=$contents_box?>px;}
...
function checkSave()
{
    ...
    if (!confirm("선택한 상품의 아이콘을 적용 하시겠습니까?")) {

        return false;

    }

    f.action = "./icon_all_update.php";
    f.submit();
...
function listUpdate(n)
{
    ...
    if (!confirm("해당 상품의 아이콘을 적용 하시겠습니까?")) {
    ...
    document.getElementById("chk_"+n).checked = true;
...
<form method="get" name="formSearch" action="icon_all.php" onSubmit="return listSearch('');" autocomplete="off">
...
<select id="f" name="f" class="select">
    ...
    <td width="4"></td>
    <td><a href="./icon_all.php"><img src="<?=$shop['image_path']?>/adm/reset.gif" border="0"></a></td>
    <td width="20"></td>
</tr>
</table>
    </td>
...
<tr height="30">
    ...
    <td class="bc1"></td>
    <td class="boxtitle">혜택별 아이콘 적용</td>
    <td class="bc1"></td>
...
<tr height="62">
    <td></td>
    <td class="chk_id"><input type="checkbox" id="chk_<?=$i?>" name="chk_id[]" value="<?=$i?>" class="checkbox" /></td>
    <td class="bc1"></td>
...
for ($k=0; $k<count($icons); $k++) {
    ...
    if (stristr($list['item_icon'], "|".$icons[$k]['id']."|")) {
...
<? if ($k > '0') { ?>
    <td width="20"></td>
<? } ?>
    <td><input type="checkbox" id="icon_insert_<?=$icons[$k]['id']?>_[<?=$i?>]" name="icon_insert[<?=$i?>][<?=$icons[$k]['id']?>]" value="1" class="checkbox" <? if ($checked) { echo 'checked'; } ?> /></td>
    <td width="5"></td>
    <td class="text1" onclick="shopElementCheckID('icon_insert_<?=$icons[$k]['id']?>_[<?=$i?>]');"><?=text($icons[$k]['title'])?></td>
<? } ?>
</tr>
</table>
    </td>
    <td class="bc1"></td>
    <td align="center"><a href="#" onclick="listUpdate('<?=$i?>'); return false;"><img src="<?=$shop['image_path']?>/adm/list_update.gif" border="0"></a></td>
</tr>
<tr><td colspan="<?=$colspan?>" height="1" class="bc1"></td></tr>
<? } ?>
</table>
...
<tr>
    ...
    <td width="5"></td>
    <td><a href="./icon_all.php"><img src="<?=$shop['image_path']?>/adm/cancel.gif" border="0"></a></td>
</tr>
</table>
