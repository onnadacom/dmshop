...
if (is_uploaded_file($tmp_name) || $file_del) {
    ...
    // 데이터
    $file = shop_popup($popup_id);

    if ($file['upload_file']) {

        // 파일 경로
        $file_path = $shop['path']."/data/popup/".shop_data_path("u", $file['upload_datetime'])."/".$file['upload_file'];
        ...
        if (file_exists($file_path) && $file['upload_file']) {
            ...
            $sql_common = "";
            $sql_common .= " set upload_source = '' ";
            $sql_common .= ", upload_file = '' ";
            $sql_common .= ", upload_filesize = '' ";
            $sql_common .= ", upload_width = '' ";
            $sql_common .= ", upload_height = '' ";
            $sql_common .= ", upload_type = '' ";
            $sql_common .= ", upload_datetime = '' ";

            // 파일 데이터 삭제
            sql_query(" update $shop[popup_table] $sql_common where id = '".addslashes($popup_id)."' ");
...
if (is_uploaded_file($tmp_name)) {
    ...
    $sql_common .= ", upload_datetime = '".$shop['time_ymdhis']."' ";
