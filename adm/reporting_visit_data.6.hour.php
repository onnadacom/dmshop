...
for ($i=0; $i<count($list); $i++) {
    ...
    for ($k=0; $k<count($report); $k++) {
        ...
        $data = sql_fetch(" select count(vi_os) as report_count from $shop[visit_table] where substring(vi_datetime,12,2) = '".$list[$i]['hour']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_os = '".addslashes($report[$k]['vi_os'])."' and vi_first = '1' group by vi_os ");
