...
// 검색조건
$sql_search = " where order_payment != '0' and order_number = '0' and order_exchange != '0' ";
...
if ($order_type == '400401') {

    $sql_search .= " and order_type in ('400','401') ";
...
function checkOk()
{
    ...
    if (!confirm("선택한 내역을 교환승인하시겠습니까?\n\n교환접수된 상태의 주문내역만 변경되며, 승인된 내역은 복구할 수 없습니다.")) {

        return false;

    }

    f.action = "./order_exchange_list_update.php";
    f.submit();
...
function checkCancel()
{
    ...
    if (!confirm("선택한 내역을 교환거절하시겠습니까?\n\n교환접수된 상태의 주문내역만 변경됩니다.")) {
...
function checkDelete()
{
    ...
    if (!confirm("선택한 내역을 삭제하시겠습니까?\n\n교환완료된 상태의 주문내역만 삭제가 되며, 삭제된 내역은 복구할 수 없습니다.")) {
...
function listUpdate(m, order_code)
{
    ...
    if (m == 'ok') {
        ...
        var message = "해당 내역을 교환승인하시겠습니까?\n\n교환접수된 상태의 주문내역만 변경되며, 승인된 내역은 복구할 수 없습니다.";

    } else {

        var message = "해당 내역을 교환거절하시겠습니까?\n\n교환접수된 상태의 주문내역만 변경됩니다.";
...
<form method="get" name="formSearch" action="order_exchange_list.php" onSubmit="return listSearch('');" autocomplete="off">
...
<select id="order_type" name="order_type" size="1" class="select" onchange="listSearch('sort');">
    ...
    <option value="400401"><?=shop_order_type("400");?>+<?=shop_order_type("401");?></option>
    <option value="900">교환완료</option>
</select>
...
<select id="f" name="f" class="select">
    ...
    <td width="4"></td>
    <td><a href="./order_exchange_list.php?order_type=400401"><img src="<?=$shop['image_path']?>/adm/reset.gif" border="0"></a></td>
    <td width="20"></td>
</tr>
</table>
    </td>
...
?>
    ...
    <td class="bc1"></td>
    <td align="center"><p class="datetime1"><?=date("Y-m-d", strtotime($list['order_exchange_datetime']));?></p><p class="datetime2"><?=date("H시 : i분", strtotime($list['order_exchange_datetime']));?></p></td>
    <td class="bc1"></td>
    <td class="none">
<table border="0" cellspacing="0" cellpadding="0">
<tr>
...
if ($list['order_cancel'] != '2' && $list['order_exchange'] != '2' && $list['order_refund'] != '2') {
    ...
    // 교환접수
    if ($list['order_exchange'] == '1') {

        echo "<td><a href='#' onclick=\"orderManage('exchange', '".$list['order_code']."'); return false;\"><img src='".$shop['image_path']."/adm/list_cancel.gif' border='0'></a></td>";
...
<tr>
    <td><a href="#" onclick="checkOk(); return false;"><img src="<?=$shop['image_path']?>/adm/all_order_exchange_ok.gif" border="0" /></a></td>
    <td width="5"></td>
    <td><a href="#" onclick="checkCancel(); return false;"><img src="<?=$shop['image_path']?>/adm/all_order_exchange_cancel.gif" border="0" /></a></td>
    <td width="5"></td>
    <td><a href="#" onclick="checkDelete(); return false;"><img src="<?=$shop['image_path']?>/adm/all_order_exchange_delete.gif" border="0" /></a></td>
    <td width="5"></td>
    <td><a href="#" onclick="checkExcel(); return false;"><img src="<?=$shop['image_path']?>/adm/all_excel.gif" border="0" /></a></td>
    <td width="5"></td>
    <td><a href="./order_excel.php?m=exchange"><img src="<?=$shop['image_path']?>/adm/excel.gif" border="0"></a></td>
</tr>
</table>
...
<table cellpadding="0" cellspacing="0" border="0" class="auto">
<tr>
    <td><img src="<?=$shop['image_path']?>/adm/guide_order_exchange.gif" border="0" usemap="#guide_map"></td>
</tr>
</table>

<map name="guide_map"><area shape="rect" coords="700,8,779,31" href="#"></map>
