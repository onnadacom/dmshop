...
// 기간의 상위 추천인을 뽑는다
$report = array();
$result = sql_query(" select *, count(*) as report_count from $shop[user_table] where substring(datetime,1,10) >= '".$date1."' and substring(datetime,1,10) <= '".$date2."' and user_recommend != '' group by user_recommend order by report_count desc limit 0, 10 ");
...
for ($i=0; $i<count($list); $i++) {
    ...
    // 해당 기간의 추천인총수
    $order = sql_fetch(" select count(*) as report_count from $shop[user_table] where substring(datetime,1,10) = '".$list[$i]['date']."' and user_recommend != '' ");

    $list_etc = 0;
    for ($k=0; $k<count($report); $k++) {

        // 해당 기간의 추천인총수을 구한다
        $data = sql_fetch(" select count(*) as report_count from $shop[user_table] where substring(datetime,1,10) = '".$list[$i]['date']."' and user_recommend = '".addslashes($report[$k]['user_recommend'])."' ");
