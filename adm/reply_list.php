...
if ($reply_score) { $reply_score = preg_match("/^[0-9]+$/", $reply_score) ? $reply_score : ""; }
if ($reply_reply) { $reply_reply = preg_match("/^[0-9]+$/", $reply_reply) ? $reply_reply : ""; }
$top_id = "2";
$left_id = "4";
$menu_id = "102";
$shop['title'] = "상품평 내역";
...
if ($reply_score) {
    ...
    $sql_search .= " and reply_score = '".$reply_score."' ";
...
if ($reply_reply) {
    ...
    if ($reply_reply == '1') {
        ...
        $sql_search .= " and reply_view = '0' ";

    }

    if ($reply_reply == '2') {

        $sql_search .= " and reply_count = '0' ";

    }

    if ($reply_reply == '3') {

        $sql_search .= " and (reply_view = '0' or reply_count = '0') ";

    }

    if ($reply_reply == '4') {

        $sql_search .= " and reply_count = '1' ";

    }

}

$cnt = sql_fetch(" select count(*) as cnt from $shop[reply_table] $sql_search ");
...
$shop_pages = shop_paging_v0("10", $page, $total_page, "?category=".$category."&reply_score=".$reply_score."&reply_reply=".$reply_reply."&date1=".$date1."&date2=".$date2."&sort=".$sort."&rows=".$rows."&f=".$f."&q=".$q."&page=");
...
$result = sql_query(" select * from $shop[reply_table] $sql_search order by $sort limit $from_record, $rows ");
...
function checkDelete()
{
    ...
    f.action = "./reply_list_update.php";
    f.submit();

}

function listDelete(reply_id)
{
...
<form method="get" name="formSearch" action="reply_list.php" onSubmit="return listSearch('');" autocomplete="off">
...
$("#category").val("<?=$category?>");
    ...
    <td width="4"></td>
    <td class="category2">
<select id="reply_score" name="reply_score" class="select" onchange="listSearch('sort');">
...
<script type="text/javascript">
$("#reply_score").val("<?=$reply_score?>");
</script>
    </td>
    <td width="4"></td>
    <td class="category3">
<select id="reply_reply" name="reply_reply" class="select" onchange="listSearch('sort');">
...
<select id="sort" name="sort" class="select" onchange="listSearch('sort');">
    ...
    <option value="reply_name asc">작성자 내림차순</option>
    <option value="reply_name desc">작성자 오름차순</option>
...
<select id="f" name="f" class="select">
    <option value="reply_title">상품평 제목</option>
    <option value="reply_content">상품평 내용</option>
    <option value="reply_name">작성자명</option>
    ...
    <td width="4"></td>
    <td><a href="./reply_list.php"><img src="<?=$shop['image_path']?>/adm/reset.gif" border="0"></a></td>
    <td width="20"></td>
</tr>
</table>
    </td>
...
for ($i=0; $list=sql_fetch_array($result); $i++) {
    ...
    // userview
    $userview = shop_userview($list['user_id'], $list['user_name'], $list['user_email'], $list['user_homepage'], $list['reply_name']);
?>
<input type="hidden" name="reply_id[<?=$i?>]" value="<?=$list['id']?>" />
<tr height="50">
    <td></td>
    ...
    <td class="bc1"></td>
    <td align="center"><div class="star<?=$list['reply_score']?>"></div></td>
    <td class="bc1"></td>
    <td>
<table border="0" cellspacing="0" cellpadding="0">
<tr>
...
<tr>
    <td width="10"></td>
    <td class="reply_title"><?=text($list['reply_title']);?></td>
</tr>
</table>
    </td>
    <td class="bc1"></td>
...
if ($list['reply_count']) {
    ...
    $dmshop_reply_reply = shop_reply_reply($list['reply_id']);

    echo "<p class='datetime1'>".date("Y-m-d", strtotime($dmshop_reply_reply['datetime']))."</p><p class='datetime2'>".date("H시 : i분", strtotime($dmshop_reply_reply['datetime']))."</p>";

}

else if ($list['reply_view']) {
...
<tr>
<? if ($list['reply_count']) { ?>
    <td><a href="#" onclick="replyPopupView('<?=$list['id']?>'); return false;"><img src="<?=$shop['image_path']?>/adm/list_read.gif" border="0"></a></td>
<? } else { ?>
    <td><a href="#" onclick="replyPopupWrite('<?=$list['id']?>'); return false;"><img src="<?=$shop['image_path']?>/adm/list_reply.gif" border="0"></a></td>
<? } ?>
    <td width="4"></td>
