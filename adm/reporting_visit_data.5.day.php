...
for ($i=0; $i<count($list); $i++) {
    ...
    for ($k=0; $k<count($report); $k++) {
        ...
        $data = sql_fetch(" select count(vi_browser) as report_count from $shop[visit_table] where substring(vi_datetime,1,10) = '".$list[$i]['date']."' and vi_browser = '".addslashes($report[$k]['vi_browser'])."' and vi_first = '1' group by vi_browser ");
