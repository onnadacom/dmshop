...
function reportingChart()
{
   ...
   var data = google.visualization.arrayToDataTable([
    ['시간', '주문 수', '주문금액', '배송비 결제금액', '쿠폰 사용금액', '적립금 사용금액', '결제금액'],
<?
$comma = "";
for ($i=0; $i<count($list); $i++) {
    ...
    echo $comma."['".$list[$i]['hour']."',".(int)($list[$i]['total_order_count']).",".(int)($list[$i]['total_order_total_item_money']).",".(int)($list[$i]['total_order_delivery_money']).",".(int)($list[$i]['total_order_total_coupon']).",".(int)($list[$i]['total_order_cash']).",".(int)($list[$i]['total_order_pay_money'])."]";
