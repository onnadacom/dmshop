...
for ($i=0; $i<count($list); $i++) {
    ...
    // 네이버 방문
    $visit_host1 = sql_fetch(" select count(vi_ip) as total_count from $shop[visit_table] where substring(vi_datetime,1,7) = '".$list[$i]['month']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_first = '1' and (INSTR(vi_host,'naver.')) ");

    // 다음 방문
    $visit_host2 = sql_fetch(" select count(vi_ip) as total_count from $shop[visit_table] where substring(vi_datetime,1,7) = '".$list[$i]['month']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_first = '1' and (INSTR(vi_host,'daum.')) ");

    // 네이트 방문
    $visit_host3 = sql_fetch(" select count(vi_ip) as total_count from $shop[visit_table] where substring(vi_datetime,1,7) = '".$list[$i]['month']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_first = '1' and (INSTR(vi_host,'nate.')) ");

    // 야후 방문
    $visit_host4 = sql_fetch(" select count(vi_ip) as total_count from $shop[visit_table] where substring(vi_datetime,1,7) = '".$list[$i]['month']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_first = '1' and (INSTR(vi_host,'yahoo.')) ");

    // 구글 방문
    $visit_host5 = sql_fetch(" select count(vi_ip) as total_count from $shop[visit_table] where substring(vi_datetime,1,7) = '".$list[$i]['month']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_first = '1' and (INSTR(vi_host,'google.')) ");

    // 직접 방문
    $visit_host6 = sql_fetch(" select count(vi_ip) as total_count from $shop[visit_table] where substring(vi_datetime,1,7) = '".$list[$i]['month']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_first = '1' and vi_host in ('{$dmshop['doamin']}', '') ");
