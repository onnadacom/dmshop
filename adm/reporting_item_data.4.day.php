...
for ($i=0; $i<count($list); $i++) {
    ...
    // 해당 기간의 총 교환 수량
    $order = sql_fetch(" select sum(order_limit) as report_count from $shop[order_table] where substring(order_datetime,1,10) = '".$list[$i]['date']."' and order_payment = '2' and order_exchange = '2' ");

    $money_etc = 0;
    for ($k=0; $k<count($report); $k++) {

        // 해당 기간의 상품별 총 교환 수량을 구한다
        $data = sql_fetch(" select sum(order_limit) as report_count from $shop[order_table] where substring(order_datetime,1,10) = '".$list[$i]['date']."' and order_payment = '2' and order_exchange = '2' and item_id = '".$report[$k]['item_id']."' ");
