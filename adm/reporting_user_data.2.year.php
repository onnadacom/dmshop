...
for ($i=0; $i<count($list); $i++) {
    ...
    // 남자
    $man = sql_fetch(" select count(*) as report_count from $shop[user_table] where substring(datetime,1,4) = '".$list[$i]['year']."' and substring(datetime,1,10) >= '".$date1."' and substring(datetime,1,10) <= '".$date2."' and user_leave_datetime = '0000-00-00 00:00:00' and user_sex = 'M' ");

    // 여자
    $woman = sql_fetch(" select count(*) as report_count from $shop[user_table] where substring(datetime,1,4) = '".$list[$i]['year']."' and substring(datetime,1,10) >= '".$date1."' and substring(datetime,1,10) <= '".$date2."' and user_leave_datetime = '0000-00-00 00:00:00' and user_sex = 'F' ");

    // 없다
    $etc = sql_fetch(" select count(*) as report_count from $shop[user_table] where substring(datetime,1,4) = '".$list[$i]['year']."' and substring(datetime,1,10) >= '".$date1."' and substring(datetime,1,10) <= '".$date2."' and user_leave_datetime = '0000-00-00 00:00:00' and user_sex = '' ");
