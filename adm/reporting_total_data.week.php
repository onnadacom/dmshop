...
if ($c1) {
    ...
    for ($i=0; $i<count($list); $i++) {
        ...
        // 방문자 수
        $visit = sql_fetch(" select count(vi_ip) as total_count from $shop[visit_table] where dayofweek(vi_datetime) = '".$list[$i]['dayofweek']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' and vi_first = '1' ");
...
if ($c2) {
    ...
    for ($i=0; $i<count($list); $i++) {
        ...
        $order = sql_fetch(" select count(distinct order_code) as total_count from $shop[order_table] where dayofweek(order_datetime) = '".$list[$i]['dayofweek']."' and substring(order_datetime,1,10) >= '".$date1."' and substring(order_datetime,1,10) <= '".$date2."' and order_payment = '2' and order_cancel != '2' and order_refund != '2' ");
...
if ($c4) {
    ...
    for ($i=0; $i<count($list); $i++) {
        ...
        // 결제가 되고, 취소, 환불 완료가 아닌 것만
        $order = sql_fetch(" select sum(order_limit) as total_count from $shop[order_table] where dayofweek(order_datetime) = '".$list[$i]['dayofweek']."' and substring(order_datetime,1,10) >= '".$date1."' and substring(order_datetime,1,10) <= '".$date2."' and order_payment = '2' and order_cancel != '2' and order_refund != '2' ");
...
if ($c7) {
    ...
    for ($i=0; $i<count($list); $i++) {
        ...
        // 결제가 되고, 교환 완료
        $order = sql_fetch(" select count(distinct order_exchange) as total_count from $shop[order_table] where dayofweek(order_exchange_ok_datetime) = '".$list[$i]['dayofweek']."' and substring(order_exchange_ok_datetime,1,10) >= '".$date1."' and substring(order_exchange_ok_datetime,1,10) <= '".$date2."' and order_payment = '2' and order_exchange = '2' ");
...
if ($c8 || $c27) {
    ...
    for ($i=0; $i<count($list); $i++) {
        ...
        // 적립금 사용 전체 내역
        $cash = sql_fetch(" select count(*) as total_count, sum(cash) as total_cash from $shop[cash_table] where dayofweek(datetime) = '".$list[$i]['dayofweek']."' and substring(datetime,1,10) >= '".$date1."' and substring(datetime,1,10) <= '".$date2."' and cash < '0' ");
...
if ($c11) {
    ...
    for ($i=0; $i<count($list); $i++) {
        ...
        // 결제가 되고, 취소, 환불 완료가 아닌 것, 쿠폰 아이디가 있는 것
        $coupon = sql_fetch(" select count(*) as total_count from $shop[coupon_list_table] where dayofweek(datetime) = '".$list[$i]['dayofweek']."' and substring(datetime,1,10) >= '".$date1."' and substring(datetime,1,10) <= '".$date2."' ");
...
if ($c13) {
    ...
    for ($i=0; $i<count($list); $i++) {
        ...
        // 해당 기간일 이전의 회원 수
        $user = sql_fetch(" select count(*) as total_count from $shop[user_table] where dayofweek(datetime) = '".$list[$i]['dayofweek']."' and substring(datetime,1,10) >= '".$date1."' and substring(datetime,1,10) <= '".$date2."' and user_level >= '2' ");
...
if ($c16) {
    ...
    for ($i=0; $i<count($list); $i++) {
        ...
        // 해당 기간의 탈퇴 회원 수
        $user = sql_fetch(" select count(*) as total_count from $shop[user_table] where dayofweek(user_leave_datetime) = '".$list[$i]['dayofweek']."' and substring(user_leave_datetime,1,10) >= '".$date1."' and substring(user_leave_datetime,1,10) <= '".$date2."' ");
...
if ($c17) {
    ...
    for ($i=0; $i<count($list); $i++) {
        ...
        // 해당 기간의 로그인 회원 수
        $user = sql_fetch(" select count(distinct user_id) as total_count from $shop[user_login_table] where dayofweek(datetime) = '".$list[$i]['dayofweek']."' and substring(datetime,1,10) >= '".$date1."' and substring(datetime,1,10) <= '".$date2."' ");
...
if ($c36) {
    ...
    for ($i=0; $i<count($list); $i++) {
        ...
        // 방문자 수
        $visit = sql_fetch(" select count(*) as total_count from $shop[visit_table] where dayofweek(vi_datetime) = '".$list[$i]['dayofweek']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' ");
...
<? if ($c4) { ?>data.addColumn('number', '주문 상품수');<? } ?>
...
for ($i=0; $i<count($list); $i++) {
    ...
    echo $comma."['".shop_week($list[$i]['week'])."요일'".$c1.$c36.$c2.$c3.$c4.$c5.$c6.$c7.$c8.$c9.$c10.$c11.$c12.$c13.$c14.$c15.$c16.$c17.$c18.$c19.$c20.$c21.$c22.$c23.$c24.$c25.$c26.$c27.$c28.$c29.$c30.$c31.$c32.$c33.$c34.$c35."]";
