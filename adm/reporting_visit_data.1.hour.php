...
for ($i=0; $i<(int)(24); $i++) {
    ...
    $list[$i]['href'] = "./reporting_visit_list.php?date1=".$date1."&date2=".$date2."&hour=".$list[$i]['hour'];
...
for ($i=0; $i<count($list); $i++) {
    ...
    // 페이지 뷰
    $visit_page = sql_fetch(" select count(*) as report_count from $shop[visit_table] where substring(vi_datetime,12,2) = '".$list[$i]['hour']."' and substring(vi_datetime,1,10) >= '".$date1."' and substring(vi_datetime,1,10) <= '".$date2."' ");

    // 해당 기간의 가입자 수
    $user = sql_fetch(" select count(*) as report_count from $shop[user_table] where substring(datetime,12,2) = '".$list[$i]['hour']."' and substring(datetime,1,10) >= '".$date1."' and substring(datetime,1,10) <= '".$date2."' and user_level >= '2' ");
...
function reportingChart()
{
   ...
   var data = google.visualization.arrayToDataTable([
    ['시간', '방문자 수', '페이지 뷰', '신규 가입자 수', '회원 가입율'],
<?
$comma = "";
for ($i=0; $i<count($list); $i++) {
    ...
    echo $comma."['".$list[$i]['hour']."',".(int)($list[$i]['total_visit_count']).",".(int)($list[$i]['total_visit_page_count']).",".(int)($list[$i]['total_user_regist_count']).",".$list[$i]['total_visit_regist_count']."]";
