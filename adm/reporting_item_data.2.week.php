...
for ($i=0; $i<count($list); $i++) {
    ...
    // 해당 기간의 판매량총수
    $order = sql_fetch(" select sum(order_limit) as report_count from $shop[order_table] where dayofweek(order_datetime) = '".$list[$i]['dayofweek']."' and substring(order_datetime,1,10) >= '".$date1."' and substring(order_datetime,1,10) <= '".$date2."' and order_payment = '2' and order_cancel != '2' and order_refund != '2' ");

    $money_etc = 0;
    for ($k=0; $k<count($report); $k++) {

        // 해당 기간의 상품별 판매량총수을 구한다
        $data = sql_fetch(" select sum(order_limit) as report_count from $shop[order_table] where dayofweek(order_datetime) = '".$list[$i]['dayofweek']."' and substring(order_datetime,1,10) >= '".$date1."' and substring(order_datetime,1,10) <= '".$date2."' and order_payment = '2' and order_cancel != '2' and order_refund != '2' and item_id = '".$report[$k]['item_id']."' ");
