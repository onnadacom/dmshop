...
for ($i=0; $i<count($list); $i++) {
    ...
    // 결제가 되고, 취소, 환불 완료가 아닌 것만
    $order_money = sql_fetch(" select count(*) as total_count, sum(item_money * order_limit) as total_item_money, sum(option_money * order_limit) as total_option_money from $shop[order_table] where substring(order_datetime,1,10) = '".$list[$i]['date']."' and order_payment = '2' and order_cancel != '2' and order_refund != '2' ");
...
for ($i=0; $i<count($list); $i++) {
    ...
    echo $comma."['".$list[$i]['date']."',".(int)($list[$i]['total_order_count']).",".(int)($list[$i]['total_item_money']).",".(int)($list[$i]['total_option_money']).",".(int)($list[$i]['total_order_total_item_money'])."]";
