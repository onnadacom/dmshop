...
if (!$plan_id) {
    ...
    alert_close("기획전이 삭제되었거나 존재하지 않습니다.");
...
$shop_pages = shop_paging_v0("10", $page, $total_page, "?plan_id=".$plan_id."&category1=".$category1."&category2=".$category2."&category3=".$category3."&category4=".$category4."&item_icon=".$item_icon."&sort=".$sort."&rows=".$rows."&f=".$f."&q=".$q."&page=");
...
function checkSave()
{
    ...
    if (!confirm("선택한 상품을 기획전에 추가 하시겠습니까?")) {
...
function listAdd(item_id)
{
    ...
    if (!confirm("해당 상품을 기획전에 추가 하시겠습니까?")) {
...
<tr>
    ...
    <td><span class="bigtitle2">[<?=text($dmshop_plan['title'])?>]</span> <span class="bigtitle">기획전 - 전시상품 추가</span></td>
</tr>
</table>
...
<form method="get" name="formSearch" action="plan_item_write.php" onSubmit="return listSearch('');" autocomplete="off">
<input type="hidden" name="plan_id" value="<?=$plan_id?>" />
<table width="100%" border="0" cellspacing="0" cellpadding="0">
...
<select id="f" name="f" class="select">
    ...
    <td width="4"></td>
    <td><a href="./plan_item_write.php?plan_id=<?=$plan_id?>"><img src="<?=$shop['image_path']?>/adm/reset.gif" border="0"></a></td>
    <td width="20"></td>
</tr>
</table>
    </td>
...
<script type="text/javascript">$(document).ready( function() { $(".sort select").selectBox(); });</script>
<script type="text/javascript">$(document).ready( function() { $(".limit select").selectBox(); });</script>
<script type="text/javascript">$(document).ready( function() { $(".icons select").selectBox(); });</script>
<script type="text/javascript">$(document).ready( function() { $(".field select").selectBox(); });</script>
...
for ($i=0; $list=sql_fetch_array($result); $i++) {
    ...
    // 기획전 상품
    $dmshop_plan_item = shop_plan_item($plan_id, $list['id']);
...
<tr height="62">
    <td></td>
    <td class="chk_id"><? if ($item_check) { ?><img src="<?=$shop['image_path']?>/adm/check2.gif"><? } else { ?><input type="checkbox" name="chk_id[]" value="<?=$i?>" class="checkbox" /><? } ?></td>
    <td class="bc1"></td>
    <td align="center"><div style="width:50px; height:50px; border:2px solid #e4e4e4;"><img src="<?=shop_item_thumb($list['id'], "default", "", "50", "50", 2);?>"></div></td>
    <td class="bc1"></td>
    <td>
<table border="0" cellspacing="0" cellpadding="0">
<tr>
...
<tr>
    ...
    <td class="bc1"></td>
    <td align="center" class="check_add"><? if ($item_check) { ?>등록중<? } else { ?><a href="#" onclick="listAdd('<?=$list['id']?>'); return false;"><img src="<?=$shop['image_path']?>/adm/list_add.gif" border="0"></a><? } ?></td>
</tr>
<tr><td colspan="<?=$colspan?>" height="1" class="bc1"></td></tr>
<? } ?>
</table>
...
<tr>
    <td class="msg2">체크박스상 선택된 모든상품이 기획전에 추가 합니다.</td>
</tr>
</table>
</form>
