...
function reportingChart()
{
   ...
   var data = google.visualization.arrayToDataTable([
    ['요일', '주문 수', '결제금액', '배송비 지출금액', '신용카드 수수료', '계좌이체 수수료', '휴대폰결제 수수료', '가상계좌 수수료', '추정매출'],
<?
$comma = "";
for ($i=0; $i<count($list); $i++) {
    ...
    echo $comma."['".shop_week($list[$i]['week'])."요일',".(int)($list[$i]['total_order_count']).",".(int)($list[$i]['total_order_pay_money']).",".(int)($list[$i]['total_order_delivery_money']).",".(int)($list[$i]['total_order_pay_type1_commission']).",".(int)($list[$i]['total_order_pay_type2_commission']).",".(int)($list[$i]['total_order_pay_type3_commission']).",".(int)($list[$i]['total_order_pay_type4_commission']).",".(int)($list[$i]['total_sales_money'])."]";
