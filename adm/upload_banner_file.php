...
if (is_uploaded_file($tmp_name) || $file_del) {
    ...
    if ($file['upload_file']) {
        ...
        // 파일 경로
        $file_path = $shop['path']."/data/banner/".shop_data_path("u", $file['upload_datetime'])."/".$file['upload_file'];
        ...
        if (file_exists($file_path) && $file['upload_file']) {
            ...
            // 파일 데이터 삭제
            sql_query(" update $shop[banner_table] $sql_common where id = '".addslashes($banner_id)."' ");
