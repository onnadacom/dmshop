...
for ($i=0; $i<count($list); $i++) {
    ...
    // 해당 기간의 키워드 총합
    $visit = sql_fetch(" select count(vi_keyword) as report_count from $shop[visit_table] where substring(vi_datetime,1,10) = '".$list[$i]['date']."' and vi_keyword != '' ");

    $list_count = 0;
    for ($k=0; $k<count($report); $k++) {

        // 해당 기간의 키워드별 카운트를 구한다
        $data = sql_fetch(" select count(vi_keyword) as report_count from $shop[visit_table] where substring(vi_datetime,1,10) = '".$list[$i]['date']."' and vi_keyword = '".addslashes($report[$k]['vi_keyword'])."' group by vi_keyword ");
