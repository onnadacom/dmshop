...
// 디스플레이 파일업로드
$file_name = "display".$display_id."_".$display_type."_".$display_list."_file";
$file_del = $_POST["display".$display_id."_".$display_type."_".$display_list."_file_del"];

$upload_mode = $file_name;
...
if (is_uploaded_file($tmp_name) || $file_del) {
    ...
    // 데이터
    $file = sql_fetch(" select * from $shop[display_box_file_table] where upload_mode = '".addslashes($upload_mode)."' ");

    if ($file['upload_file']) {

        // 파일 경로
        $file_path = $shop['path']."/data/display_box/".shop_data_path("u", $file['datetime'])."/".$file['upload_file'];
        ...
        // 삭제
        sql_query(" delete from $shop[display_box_file_table] where upload_mode = '".addslashes($upload_mode)."' ");
...
if (is_uploaded_file($tmp_name)) {
    ...
    // 등록
    sql_query(" insert into $shop[display_box_file_table] $sql_common ");
