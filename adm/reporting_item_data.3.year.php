...
for ($i=0; $i<count($list); $i++) {
    ...
    // 해당 기간의 총 취소 수량
    $order = sql_fetch(" select sum(order_limit) as report_count from $shop[order_table] where substring(order_datetime,1,4) = '".$list[$i]['year']."' and substring(order_datetime,1,10) >= '".$date1."' and substring(order_datetime,1,10) <= '".$date2."' and order_payment = '2' and order_cancel = '2' ");

    $money_etc = 0;
    for ($k=0; $k<count($report); $k++) {

        // 해당 기간의 상품별 총 취소 수량을 구한다
        $data = sql_fetch(" select sum(order_limit) as report_count from $shop[order_table] where substring(order_datetime,1,4) = '".$list[$i]['year']."' and substring(order_datetime,1,10) >= '".$date1."' and substring(order_datetime,1,10) <= '".$date2."' and order_payment = '2' and order_cancel = '2' and item_id = '".$report[$k]['item_id']."' ");
