...
if ($m == 'refresh_token') {
    ...
    if ($callback == 'hidden') {
        ...
        $kakao_refresh_token = shop_get_session("kakao_refresh_token");
        ...
        $curl = new Curl();
        $curl->post('https://kauth.kakao.com/oauth/token', array(
            'client_id' => addslashes($dmshop['login_kakao_key']),
            'refresh_token' => addslashes($kakao_refresh_token),
        ...
        shop_set_session("kakao_access_token", $curl->response->access_token);
...
if ($code) {
    ...
    $curl->post('https://kauth.kakao.com/oauth/token', array(
        ...
        'redirect_uri' => addslashes($shop['url']."/login/kakao.php"),
    ...
    shop_set_session("kakao_access_token", $curl->response->access_token);
    shop_set_session("kakao_refresh_token", $curl->response->refresh_token);

    url("kakao.php");

}

else if (shop_get_session("kakao_access_token") && $m != 're') {

    $curl = new Curl();
    $curl->setHeader('Authorization', "Bearer ".addslashes(shop_get_session("kakao_access_token")));
    $curl->get('https://kapi.kakao.com/v1/user/me');

    if (!$curl->response->id) {

        message("<p class='title'>알림</p><p class='text'>오류코드 : ".text($curl->response->code)."<br />오류내용 : ".text($curl->response->msg)."</p>", $message_mode);

    }

    $social_key = addslashes($curl->response->id);
    $nick = addslashes($curl->response->properties->nickname);
    $photo = addslashes($curl->response->properties->profile_image);
    $user_id = "dmk".(int)($dmshop['login_kakao_count'] + 1);
    ...
    $chk = sql_fetch(" select * from $shop[user_table] where social = 2 and social_key = '".$social_key."' limit 0, 1 ");
    ...
    if ($chk['user_id']) {
        ...
        $sql_common = "";
        $sql_common .= " set login_kakao_count = login_kakao_count + 1 ";
    ...
    $url .= "?redirect_uri=".urlencode($shop['url']."/login/kakao.php");
    $url .= "&response_type=code";
    $url .= "&client_id=".addslashes($dmshop['login_kakao_key']);
    $url .= "&state=".$state;
