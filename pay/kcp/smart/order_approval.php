...
$siteCode         = text($_GET[ "site_cd"     ]);
$orderID          = text($_GET[ "ordr_idxx"   ]);
$paymentMethod    = text($_GET[ "pay_method"  ]);
$escrow           = ( $_GET[ "escw_used"   ] == "Y" ) ? true : false;
$productName      = text($_GET[ "good_name"   ]);

// 아래 두값은 POST된 값을 사용하지 않고 서버에 SESSION에 저장된 값을 사용하여야 함.
$paymentAmount    = text($_GET[ "good_mny"    ]); // 결제 금액
$returnUrl        = text($_GET[ "Ret_URL"     ]);
